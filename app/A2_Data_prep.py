...
print("########################################")
print("# I- Préparation données US_aves.parquet")
print("########################################")
print(" ")
print(" ")
...
# 2. Exploration du fichier avant preparation des données
print("Aperçu du fichier")
print(df_obs.tail(20))
print(" ")
...
# 2. Contrôle du nombre de lignes de données du fichier
nb_lignes_init_US_aves = df_obs.count()
print("Nombre de lignes dans le fichier US_aves (initial) :", nb_lignes_init_US_aves)
print(" ")

# 3. Vérification de l'unicité des id observations dans le dataframe
...
print("Unicité des id observations : ", Verif_id_uniques_obs )
print(" ")

# 4. Vérification de l'existence ou non de lignes avec taxons vides dans le dataframe :

df_obs = df_obs.dropna(subset=["taxonID"])
nb_lignes_US_aves = df_obs.count()

print("Nombre de lignes après suppression des lignes avec id taxons non renseignés :", nb_lignes_US_aves)
print(" ")

print("Nombre de lignes supprimées à cause de id taxons non renseignés :", nb_lignes_init_US_aves - nb_lignes_US_aves)
print(" ")

# 5. Uniformisation des dates renseignées dans la colonne eventDate
...
#Supression de la colnne eventDate pour la recréer avec les données extraites, dont on est maintenant sur qu'elles sont formatées
df_obs = df_obs.drop('eventDate')

#Recalcul de la colonne EventDate à partir des données extraites
...
#Calcul d'une colonne Année - Mois
df_obs = df_obs.withColumn(
    'yearMonthEvent', 
    when(
...
# Convertir la colonne 'eventDate' en une colonne de type date au format souhaité
df_obs = df_obs.withColumn('eventDate', date_format('eventDate', 'yyyy-MM-dd'))

# Convertir la colonne 'yearMonthEvent' en une colonne de type date au format souhaité
df_obs = df_obs.withColumn('yearMonthEvent', date_format('yearMonthEvent', 'yyyy-MM-dd'))

# Suppressions de la colonne dayEvent non utile dans le dataframe
df_obs = df_obs.drop('dayEvent')

print("Nombre de lignes de la colonne yearEvent nulles :", df_obs.filter(df_obs["yearEvent"].isNull()).count())
print("Nombre de lignes de la colonne monthEvent nulles :", df_obs.filter(df_obs["monthEvent"].isNull()).count())
print("Nombre de lignes de la colonne eventDate nulles :", df_obs.filter(df_obs["eventDate"].isNull()).count())
print("Nombre de lignes de la colonne yearMonthEvent nulles :", df_obs.filter(df_obs["yearMonthEvent"].isNull()).count())
...
print("Les lignes avec une date manquante ou mal renseignée ont été supprimées. Total des lignes supprimées :", nb_lignes_US_aves - nb_lignes_US_aves_post_drop_dates_nulles)
print(" ")


# 6. Sélection de la plage de dates étudiées 

# Création d'un graph pour voir graphiquement l'étendue de la plage de dates
...
plotly_parannee.update_layout(
    title='Graphique en barres du nombre d\'observations par année',
    xaxis_title='Années',
    yaxis_title='Nombre d\'observations'
)

plotly_parannee.write_image('/app/A2_1_Diagramme_nb_obs_par_annee_plotly.png')
...
print("Nombre de lignes supprimées pre 2017 et post 2022 :", nb_lignes_US_aves_post_drop_dates_nulles - nb_lignes_US_aves_post_drop_dates)
print(" ")
print("Nouveau décompte de lignes :", nb_lignes_US_aves_post_drop_dates)
print(" ")



# 6. Vérifications valeurs nulles latitude / longitude
print("Nombre de lignes de la colonne decimalLatitude nulles :", df_obs.filter(df_obs["decimalLatitude"].isNull()).count())
print(" ")
print("Nombre de lignes de la colonne decimalLongitude nulles :", df_obs.filter(df_obs["decimalLongitude"].isNull()).count())
print(" ")

df_obs = df_obs.dropna(subset=["decimalLatitude"])
df_obs = df_obs.dropna(subset=["decimalLongitude"])

nb_lignes_US_aves_post_drop_lat_long_nulles = df_obs.count()

print("Les lignes avec latitude ou longitude manquante ou mal renseignée ont été supprimées. Total des lignes supprimées :", nb_lignes_US_aves_post_drop_dates - nb_lignes_US_aves_post_drop_lat_long_nulles)
print(" ")




# 7. Vérification du nombre de lignes final
...
print("################################################")
print("# II- Préparation fichier Noms_especes.parquet")
print("################################################")
print(" ")
print(" ")
...
# Sélectionnez la première occurrence de chaque groupe d'ID avec son libellé correspondant
...
print("########################################")
print("# III- Préparation Noms_Etats.parquet   ")
print("########################################")
print(" ")
print(" ")
...
# jointure gauche pour ne pas perdre de données si certaines observations n'ont pas leurs noms français de renseignés
