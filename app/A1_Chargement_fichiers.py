...
########################################
# III - Fichier Pop surface
########################################

# 1. Création du schéma de données pour le fichier Noms_Etats
schemaStatePop = StructType([StructField('State_Code', StringType() , True)
                    , StructField('Surface', IntegerType() , True)
                    , StructField('Population', IntegerType() , True)])

# 2. Chargement du fichier CSV Noms_Etats en tant que DataFrame
df_StatePop = spark.read.format("csv").option("header", "true").option("delimiter",";").schema(schemaStatePop).load("/app/surface_pop.csv")

# 3. Transformation et enregistrement du dataframe df_StateNames en tant que fichier Parquet
df_StatePop.write.mode("overwrite").format("parquet").save("/app/Pop_Etat.parquet")
