...
df_names = spark.read.format("parquet").load("/app/conv_noms_pays.parquet")
...
# 1. Chargement des fichiers Parquet US_aves en tant que DataFrame spark
df_obs_family = spark.read.format("parquet").load("/app/WORLD_month_country_family.parquet")


#Jointure entre les 2 fichiers, sur l'ID taxon
nb_lignes_avant_join = df_obs_family.count()

print("Nombre lignes dans dataframe df_obs_family avant jointure :",nb_lignes_avant_join)
print(" ")

# jointure gauche pour ne pas perdre de données si certaines observations n'ont pas leurs noms français de renseignées
df_obs_family = df_obs_family.join(df_names, df_obs_family["CountryCode"] == df_names["code_inat"], "left")

nb_lignes_après_join = df_obs_family.count()
print("Nombre lignes dans dataframe df_obs_family après jointure :", nb_lignes_après_join)
print(" ")
...
# df_obs_family = df_obs_family.dropna(subset=["family"])
df_obs_family.show()


df_obs_family.write.mode("overwrite").format("parquet").save("/app/WORLD_month_country_family_names.parquet")
