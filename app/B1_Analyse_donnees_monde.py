...
###########################################
# I - conv_noms_pays.csv
###########################################

# 1. Création du schéma de données pour le fichier conv_noms_pays.csv
...
# 2. Chargement du fichier CSV conv_noms_pays en tant que DataFrame
df_nomspays = spark.read.format("csv").option("header", "true").option("delimiter","|").schema(schemaNoms).load("/app/conv_noms_pays.csv")


# 3. Transformation et enregistrement du dataframe df en tant que fichier Parquet
df_nomspays.write.mode("overwrite").format("parquet").save("/app/conv_noms_pays.parquet")
